...
4. **工具调用是核心能力**: 通过 bind_tools 实现
5. **结构化输出提高可靠性**: 使用 with_structured_output
6. **流式处理改善体验**: stream() 和 astream_events()
7. **合理配置参数**: temperature、max_tokens 等
8. **监控 token 使用**: 优化成本和性能
9. **错误处理和重试**: 提高系统稳定性
10. **根据任务选择模型**: 平衡性能、成本和速度
