...
**MCP (Model Context Protocol)** 是一个开放协议，用于标准化应用程序如何向 LLM 提供**工具**和**上下文**。它定义了一种统一的方式，让 AI 应用能够：
...
async def on_elicitation(mcp_context, params, context: CallbackContext):
    """处理征询请求"""
    # 在实际应用中，这里会提示用户输入
